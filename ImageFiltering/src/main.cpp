...
/*
* ./Assignment2 -input sample_images/cube.jpg -brightness 1.1 -output out.bmp
*/
...
int main( int argc, char* argv[] ){
    ...
    while (argc > 0)
    {
        if (**argv == '-')
        {
            ...
            else if (!strcmp(*argv, "-nonPhotorealistic"))
            {
            if (img == NULL) ShowUsage();
            ...
            else if (!strcmp(*argv, "-compression"))
            {
            if (img == NULL) ShowUsage();

            Image *dst = img->Compression();
            delete img;
            img = dst;
